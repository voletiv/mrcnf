...
class CelebA(VisionDataset):
    ...
    def __init__(self, root, split="train", target_type="attr", transform=None,
        ...
        splits = pandas.read_csv(fn("list_eval_partition.csv"),  header='infer', index_col=0)
        #identity = pandas.read_csv(fn("identity_CelebA.txt"), delim_whitespace=True, header=None, index_col=0)
        bbox = pandas.read_csv(fn("list_bbox_celeba.csv"), header='infer', index_col=0)
        landmarks_align = pandas.read_csv(fn("list_landmarks_align_celeba.csv"),  header='infer', index_col=0)
        attr = pandas.read_csv(fn("list_attr_celeba.csv"), header='infer', index_col=0)

        mask = slice(None) if split is None else (splits['partition'] == split)


        self.filename = splits[mask].index.values
        #self.identity = torch.as_tensor(identity[mask].Ivalues)
